...
# Image
new = Image.PhotoImage(Image.open('logo.png'))
...
if int((dt.strftime('%I'))) >= 8 & int((dt.strftime('%I'))) <= 5:
    img = Image.PhotoImage(image=PIL.Image.fromarray('moon.png'))
    panel = Label(root, image=img)
    panel.place(x=210, y=200)
else:
    img = Image.PhotoImage(image=PIL.Image.fromarray('sun.png'))
    panel = Label(root, image=PIL.img)
    panel.place(x=210, y=200)
